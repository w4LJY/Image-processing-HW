...
Mat ifft_func3(Mat);
Mat ifft_show3(Mat);
Mat GaussianL_func(Mat, float);
void mse_psnr3(Mat, Mat);
...
int main6_2b()
{
	...
	Mat result5 = GaussianL_func(aerialcity, 5);
	imshow("Gaussian HPF D0=5", result5);
	printf("Gaussian HPF D0=5:");
	mse_psnr3(result5, aerialcity);
	waitKey(0);

	Mat result25 = GaussianL_func(aerialcity, 25);
	imshow("Gaussian HPF D0=25", result25);
	printf("Gaussian HPF D0=25:");
	mse_psnr3(result25, aerialcity);
	waitKey(0);

	Mat result125 = GaussianL_func(aerialcity, 125);
	imshow("Gaussian HPF D0=125", result125);
	printf("Gaussian HPF D0=125:");
	mse_psnr3(result125, aerialcity);
	waitKey(0);
...
Mat GaussianL_func(Mat I, float D0) {
	...
	for (int i = 0; i < H; i++) {
		for (int j = 0; j < W; j++) {
			float D2 = pow(i - H / 2, 2) + pow(j - W / 2, 2);//找到與dc點的距離
			dftI.at<Vec2f>(i, j)[0] = dftI.at<Vec2f>(i, j)[0] * (1 - exp(-D2 / (2 * pow(D0, 2))));
			dftI.at<Vec2f>(i, j)[1] = dftI.at<Vec2f>(i, j)[1] * (1 - exp(-D2 / (2 * pow(D0, 2))));
		}
	}
...
void mse_psnr3(Mat I, Mat original) {
	int H = I.rows, W = I.cols;
	double MSE = 0;
	for (int i = 0; i < H; i++)
		for (int j = 0; j < W; j++)
